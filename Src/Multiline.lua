...
function Multiline:Enter(text, chatType, language, target)
	...
	-- Proxy mode: the original Blizzard editbox is visible behind
	...
	if eb and eb._proxyOrigEditBox then
		pcall(function() eb._proxyOrigEditBox:Deactivate() end)
	end
...
function Multiline:Exit(restoreText, suppressOverlay)
	...
	if eb then
		...
		if not suppressOverlay then
			...
			-- Proxy mode: also re-show the underlying Blizzard editbox
